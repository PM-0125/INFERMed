...
def convert_twosides_csv(csv_path: str, out_parquet: str) -> None:
    ...
    out = out[
        (out["drug_a"] != "") & (out["drug_b"] != "") & (out["side_effect"] != "")
    ]
    # Drop self-pairs
...
def convert_offsides_csv(csv_path: str, out_parquet: str) -> None:
    ...
    cols = {c.lower(): c for c in df.columns}
    name_col = (
        cols.get("drug_concept_name") or cols.get("drug_name") or cols.get("drug")
    )
    se_col = (
        cols.get("condition_concept_name")
        or cols.get("side_effect")
        or cols.get("adverse_event")
    )
    prr_col = cols.get("prr")
    freq_col = cols.get("mean_reporting_frequency")
    if not name_col or not se_col:
        raise ValueError(
            f"OFFSIDES: could not infer drug and side-effect columns from {list(df.columns)}"
        )
    ...
    out = pd.DataFrame(
        {
            ...
            "mean_reporting_frequency": (
                pd.to_numeric(df[freq_col], errors="coerce") if freq_col else None
            ),
        }
    )
    for col in ("a", "b", "c", "d"):
...
def convert_sider_label_side_effects(
    ...
    assert (
        se_p.exists()
    ), f"SIDER label side-effects file not found: {label_side_effects_path}"

    names = pd.read_csv(
        names_p, sep="\t", header=None, names=["stitch_id", "drug_name"], dtype=str
    )
...
def _load_nci_compound_names(compound_names_path: str) -> pd.DataFrame:
    ...
    assert (
        p.exists()
    ), f"NCI-ALMANAC compound names file not found: {compound_names_path}"
...
def convert_nci_almanac_zip(
    ...
    compound_out_p = (
        Path(compound_out_parquet)
        if compound_out_parquet
        else out_p.with_name("nci_almanac_compounds.parquet")
    )
    ...
    canonical_by_nsc: Dict[str, str] = (
        compounds.drop_duplicates(subset=["nsc"], keep="first")
        .set_index("nsc")["drug_name"]
        .to_dict()
    )
    display_by_nsc: Dict[str, str] = (
        compounds.drop_duplicates(subset=["nsc"], keep="first")
        .set_index("nsc")["display_name"]
        .to_dict()
    )
    ...
    try:
        ...
        raise RuntimeError(
            "pyarrow is required for chunked NCI-ALMANAC parquet conversion"
        ) from exc
    ...
    def normalize_chunk(chunk: pd.DataFrame) -> pd.DataFrame:
        ...
        out = pd.DataFrame(
            {
                "combo_drug_seq": pd.to_numeric(
                    chunk["COMBODRUGSEQ"], errors="coerce"
                ).astype("Int64"),
                "screener": chunk["SCREENER"].map(_norm_name),
                "study": chunk["STUDY"].map(_norm_name),
                "test_date": chunk["TESTDATE"].map(_norm_name),
                "panel_nbr": pd.to_numeric(chunk["PANELNBR"], errors="coerce").astype(
                    "Int64"
                ),
                "cell_nbr": pd.to_numeric(chunk["CELLNBR"], errors="coerce").astype(
                    "Int64"
                ),
                ...
                "conc_index_a": pd.to_numeric(
                    chunk["CONCINDEX1"], errors="coerce"
                ).astype("Int64"),
                "conc_index_b": pd.to_numeric(
                    chunk["CONCINDEX2"], errors="coerce"
                ).astype("Int64"),
                ...
                "percent_growth": pd.to_numeric(
                    chunk["PERCENTGROWTH"], errors="coerce"
                ),
                "expected_growth": pd.to_numeric(
                    chunk["EXPECTEDGROWTH"], errors="coerce"
                ),
    ...
    with zipfile.ZipFile(zip_p) as zf:
        ...
        with zf.open(names[0]) as handle:
            for chunk in pd.read_csv(
                handle, usecols=usecols, chunksize=chunksize, low_memory=False
            ):
...
def convert_dictrank_excel(xlsx_path: str, out_parquet: str) -> None:
    ...
    if name_col is None:
        raise ValueError(
            f"DICTRank: Could not infer name column from {list(df.columns)}"
        )
    ...
    out.to_parquet(out_parquet, index=False)
    print(
        f"[OK] DICTRank â†’ {out_parquet}  (rows={len(out)}, with_score={out['score'].notna().sum()})"
    )
...
def convert_dilirank_excel(xlsx_path: str, out_parquet: str) -> None:
    ...
    if first_row is not None and isinstance(
        first_row.iloc[1] if len(first_row) > 1 else None, str
    ):
        ...
        for c in df.columns:
            if df[c].dtype == object and df[c].notna().sum() > 0:
                ...
                if (
                    ...
                    and sample.lower()
                    not in ["compound name", "severity class", "label section"]
                ):
                    name_col = c
                    break
    ...
    if name_col is None:
        raise ValueError(
            f"DILIRank: Could not infer name column from {list(df.columns)}"
        )
    if score_col is None:
        raise ValueError(
            f"DILIRank: Could not infer score column from {list(df.columns)}"
        )
    ...
    out.to_parquet(out_parquet, index=False)
    print(
        f"[OK] DILIRank â†’ {out_parquet}  (rows={len(out)}, with_score={out['dili_score'].notna().sum()})"
    )
...
def convert_diqt_excel(xlsx_path: str, out_parquet: str) -> None:
    ...
    if score_col is None:
        ...
        num_cols = [
            c
            for c in df.columns
            if c != name_col and c != "Pubchem_ID" and _is_numeric_series(df[c])
        ]
        if num_cols:
    ...
    if score_col is None:
        raise ValueError(
            f"DIQT: no numeric score columns found. Columns={list(df.columns)}"
        )
    ...
    out = pd.DataFrame(
        ...
        subset=["drug_name", "score"]
    )  # Only keep rows with both name and score

    Path(out_parquet).parent.mkdir(parents=True, exist_ok=True)
    out.to_parquet(out_parquet, index=False)
    print(
        f"[OK] DIQT â†’ {out_parquet}  (rows={len(out)}, with_score={out['score'].notna().sum()})"
    )
...
def _build_cli() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Convert public-safe source datasets to tidy Parquet."
    )
    ...
    offsides.add_argument(
        "--csv", required=True, help="Path to OFFSIDES.csv.xz or CSV."
    )
    offsides.add_argument("--out", required=True, help="Output Parquet file path.")

    sider = sub.add_parser(
        "sider-label-se", help="Convert SIDER label side-effect TSV files to Parquet."
    )
    sider.add_argument(
        "--drug-names", required=True, help="Path to SIDER drug_names.tsv."
    )
    sider.add_argument(
        "--label-se", required=True, help="Path to SIDER meddra_all_label_se.tsv.gz."
    )
    ...
    dictrank.add_argument(
        "--xlsx", required=True, help="Path to dictrank_dataset_508.xlsx."
    )
    ...
    dilirank.add_argument(
        "--xlsx", required=True, help="Path to dilirank_diliscore_lit.xlsx."
    )
    ...
    diqt.add_argument(
        "--xlsx", required=True, help='Path to "diqt-drug information.xlsx".'
    )
    diqt.add_argument("--out", required=True, help="Output Parquet file path.")

    nci = sub.add_parser(
        "nci-almanac", help="Convert NCI-ALMANAC combo growth zip to Parquet."
    )
    nci.add_argument(
        "--zip", required=True, help="Path to ComboDrugGrowth_Nov2017.zip."
    )
    nci.add_argument(
        "--compound-names", required=True, help="Path to ComboCompoundNames_all.txt."
    )
    nci.add_argument(
        "--out", required=True, help="Output NCI-ALMANAC Parquet file path."
    )
    nci.add_argument(
        "--compound-out",
        default=None,
        help="Output compound alias Parquet path. Defaults beside --out.",
    )
    nci.add_argument(
        "--chunksize", type=int, default=250_000, help="CSV rows per conversion chunk."
    )
