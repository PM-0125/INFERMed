...
def detect_pk_mechanisms(
    ...
    evidence_ids = [
        card.evidence_id
        for card in cards
        if card.claim_type in {"mechanism", "pk_mechanism", "mechanism_reference"}
    ]
    if not (
        pk_detail.get("canonical_interaction")
        or pkpd.get("pk_summary")
        or any((pk_detail.get("overlaps") or {}).values())
    ):
    ...
    nodes = [
        MechanismNode(
            id=mechanism_id, label="PK mechanism", type="mechanism", payload=pk_detail
        )
    ]
    edges = [
        MechanismEdge(
            source=f"drug:{pair[1]}",
            target=mechanism_id,
            type="drives",
            confidence="medium",
