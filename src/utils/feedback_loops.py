...
class FeedbackTracker:
    ...
    def __init__(self, feedback_file: str = FEEDBACK_FILE):
        ...
        self.item_scores: Dict[str, float] = defaultdict(
            lambda: 1.0
    ...
    def _update_item_scores(self):
        ...
        for item, score in saved_scores.items():
            if (
                item not in self.item_scores
                or saved_scores[item] > self.item_scores[item]
            ):
    ...
    def adjust_retrieval_weights(
        self, items: List[str], base_scores: Optional[List[float]] = None
    ) -> List[float]:
        """
