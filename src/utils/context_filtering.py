...
def filter_context_by_relevance(
    ...
    if "signals" in context:
        ...
        if "tabular" in signals:
            ...
            if len(filtered_se_a) < len(side_effects_a) or len(filtered_se_b) < len(
                side_effects_b
            ):
        ...
        if "mechanistic" in signals:
            ...
            if len(filtered_targets_a) < len(targets_a) or len(
                filtered_targets_b
            ) < len(targets_b):
            ...
            metadata["items_after"] += len(filtered_pathways_a) + len(
                filtered_pathways_b
            )
            if len(filtered_pathways_a) < len(pathways_a) or len(
                filtered_pathways_b
            ) < len(pathways_b):
...
def filter_context_sections(
    ...
    if "side_effects" in sections_to_filter and "signals" in context:
        ...
        if "tabular" in signals:
            ...
            filtered_se_a = _filter_items_by_relevance(
                se_a, query_context, "side_effect", min_relevance
            )
            filtered_se_b = _filter_items_by_relevance(
                se_b, query_context, "side_effect", min_relevance
            )
