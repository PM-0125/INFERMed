...
def calculate_result_quality(
    results: List[Any], score_extractor: Optional[Callable[[Any], float]] = None
) -> Dict[str, float]:
    """
...
def adaptive_retrieve(
    ...
    if quality["avg_score"] < min_relevance:
        should_expand = True
        LOG.debug(
            f"Low average relevance ({quality['avg_score']:.2f} < {min_relevance}), expanding"
        )
    ...
    if quality["count"] < min_results:
        should_expand = True
        LOG.debug(
            f"Insufficient results ({quality['count']} < {min_results}), expanding"
        )
    ...
    if should_expand:
        ...
        if expanded_k > initial_k:
            ...
            expanded_quality = calculate_result_quality(
                expanded_results, score_extractor
            )
