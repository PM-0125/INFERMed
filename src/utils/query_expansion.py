...
def get_best_match_from_expanded(
    expanded_terms: List[str], match_function: callable, max_results: int = 1
) -> List[str]:
    """
...
def merge_expanded_results(
    results_by_term: Dict[str, Any], merge_function: Optional[callable] = None
) -> Any:
    """
...
def expand_with_semantic_similarity(
    drug_name: str, semantic_searcher: Any, top_k: int = 3, threshold: float = 0.7
) -> List[str]:
    """
...
def create_expanded_query_context(
    ...
    expanded_a = expand_drug_query(
        drug_a, synonyms_a, use_variations=True, use_case_variants=True
    )
    expanded_b = expand_drug_query(
        drug_b, synonyms_b, use_variations=True, use_case_variants=True
    )

    # Semantic expansion (REQUIRED for RAG system)
    if semantic_searcher is None:
        raise RuntimeError(
            "Semantic searcher is required for query expansion. Please ensure semantic search is initialized."
        )

    semantic_a = expand_with_semantic_similarity(
        drug_a, semantic_searcher, top_k=5, threshold=0.6
    )
    semantic_b = expand_with_semantic_similarity(
        drug_b, semantic_searcher, top_k=5, threshold=0.6
    )
