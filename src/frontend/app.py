...
def _resp_cache_key(pair: Tuple[str, str], question: str, mode: str, temp: float, seed: int) -> str:
...
def _render_header():
    st.set_page_config(page_title=f"{APP_TITLE} — RAG · PK/PD · FAERS", layout="wide")
    st.title("INFERMed")
...
def _sidebar():
    with st.sidebar:
        ...
        st.session_state.mode = st.radio(
            "Audience mode", ["Doctor", "Patient", "Pharma"], index=["Doctor","Patient","Pharma"].index(st.session_state.mode)
        )
        st.session_state.temperature = st.slider("Creativity (temperature)", 0.0, 1.0, float(st.session_state.temperature), 0.05)
        st.session_state.seed = st.number_input("Seed (for determinism)", value=int(st.session_state.seed), step=1)
        ...
        st.markdown("---")
        st.caption("Tip: For follow-ups on the same pair, just ask below — context is preserved during this session.")

def _render_pair_inputs():
    colA, colB = st.columns(2)
    with colA:
        st.session_state.drugA = st.text_input("Drug A", value=st.session_state.drugA, key="drugA_input")
    with colB:
        st.session_state.drugB = st.text_input("Drug B", value=st.session_state.drugB, key="drugB_input")
...
def _maybe_load_context(a: str, b: str) -> Dict[str, Any]:
    ...
    with st.spinner("Retrieving evidence and normalizing (RAG)…"):
        ctx, _ = get_context_cached(a, b) if use_cache else (retrieve_and_normalize(a, b), None)
...
def _render_topline(context: Dict[str, Any]):
    ...
    pkpd = context.get("pkpd", {})
    st.caption(f'PK: {pkpd.get("pk_summary","(none)")}  \nPO: {pkpd.get("pd_summary","(none)")}')

    # Sources badges
    sources = context.get("sources", {})
    if sources:
        tags = []
        if sources.get("duckdb"): tags.append("duckdb: " + ", ".join(sources["duckdb"]))
        if sources.get("qlever"): tags.append("qlever: " + ", ".join(sources["qlever"]))
        if sources.get("openfda"): tags.append("openfda: " + ", ".join(sources["openfda"]))
        if tags:
...
def _render_evidence(context: Dict[str, Any]):
    ...
    with tabs[0]:
        ...
        st.json({
            "inhibition": pkpd_norm.get("inhibition") or [],
            "induction": pkpd_norm.get("induction") or [],
            "shared_substrate": pkpd_norm.get("shared_substrate") or [],
        })

        st.markdown("**PD overlap (targets/pathways)**")
        st.json({
            "overlap_targets": pkpd_norm.get("overlap_targets") or [],
            "overlap_pathways": pkpd_norm.get("overlap_pathways") or [],
            "pd_score": pkpd_norm.get("pd_score", 0),
        })
    ...
    with tabs[1]:
        ...
        if a_top:
            ...
            fig_a = px.bar(df_a.head(5), x="reaction", y="count", title=f"Top 5 Reactions for {a_name}")
        ...
        if b_top:
            ...
            fig_b = px.bar(df_b.head(5), x="reaction", y="count", title=f"Top 5 Reactions for {b_name}")
...
def _render_chat():
    ...
    # follow-up input
    user_msg = st.chat_input("Ask about this drug pair, e.g., 'Is dose adjustment needed?'")
    if user_msg:
...
def _handle_explain_pair():
    ...
    with st.spinner("Generating explanation…"):
        # Use run_rag so the same path is used as tests; no prior history on first turn
        out = run_rag(a, b, mode=mode, temperature=temp, seed=seed, history=[])
        text = out.get("text")
        if text is None:
            st.error("No explanation was generated. Please check the backend or logs for errors. Debug info: {}".format(out))
            return
...
def _handle_followup(user_msg: str):
    """Reuse the current context; call the LLM with history."""
    ...
    with st.spinner("Thinking…"):
        ...
        out = generate_response(context, mode, seed=seed, temperature=temp, history=history)
...
with col_btn1:
    if st.button("Explain this pair", type="primary", use_container_width=True, key="btn_explain_pair"):
...
# Show topline + evidence for the active pair (single place; no duplicate render)
