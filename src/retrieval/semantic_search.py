...
class SemanticSearcher:
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            LOG.warning(
                f"Failed to initialize embedding model; using fallback if enabled: {e}"
            )
    ...
    def build_drug_index(
        self, drug_names: List[str], force_rebuild: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            # Store in index
            self.drug_index = {
                name: emb for name, emb in zip(unique_drugs, all_embeddings)
            }
    ...
    def search_similar_drugs(
        self, query: str, top_k: int = 10, threshold: float = SIMILARITY_THRESHOLD
    ...
    def build_side_effect_index(
        self, side_effects: List[str], force_rebuild: bool = False
    ) -> bool:
        """
        ...
        if not force_rebuild and os.path.exists(cache_path):
            try:
                ...
                LOG.info(
                    f"Loaded side effect index from cache: {len(self.side_effect_names)} effects"
                )
                return True
            except Exception as e:
        ...
        try:
            ...
            self.side_effect_index = {
                name: emb for name, emb in zip(unique_effects, all_embeddings)
            }
            ...
            try:
                with open(cache_path, "wb") as f:
                    pickle.dump(
                        {
                            "names": self.side_effect_names,
                            "index": self.side_effect_index,
                        },
                        f,
                    )
                LOG.info(
                    f"Saved side effect index to cache: {len(self.side_effect_names)} effects"
                )
            except Exception as e:
