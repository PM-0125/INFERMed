        ...
        if r.status_code == 200:
            ...
            for pathway_id in pathway_ids[:10]:  # Limit to top 10
                try:
                    ...
                    LOG.debug(
                        "Reactome pathway info query failed for %s: %s", pathway_id, e
                    )
    except Exception as e:
...
@lru_cache(maxsize=256)
def get_drug_target_pathways(
    drug_name: str, target_uniprot_ids: List[str]
) -> List[Dict[str, Any]]:
    """
