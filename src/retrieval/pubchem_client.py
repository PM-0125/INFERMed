...
def get_protein_label(protein_id: str) -> Optional[str]:
    ...
    if "_" in protein_id and len(protein_id.split("_")[0]) == 4:
        ...
        chain = (
            protein_id.split("_")[1].upper() if len(protein_id.split("_")) > 1 else ""
        )
        ...
        try:
            ...
            r = requests.get(
                url, headers={"Accept": "application/rdf+xml"}, timeout=PUBCHEM_TIMEOUT
            )
    ...
    if (
        protein_id.startswith(("P", "Q", "O", "A"))
        and len(protein_id) >= 6
        and not "_" in protein_id
    ):
        try:
            _rate_limit()
        ...
        enriched.append(
            {
                ...
                "uri": (
                    pid
                    if pid.startswith("http://")
                    else f"http://rdf.ncbi.nlm.nih.gov/pubchem/protein/{clean_id}"
                ),
