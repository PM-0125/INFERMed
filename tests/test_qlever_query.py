...
def test_constants_present_and_correct():
    ...
    assert PUBCHEM_COMPOUND_NS.startswith(
...
def test_core_label_fragment_contains_aspirin(core_client):
    pairs = _skip_on_qlever_hiccup(ql.core_find_cid_by_label_fragment, "aspirin", 20)
...
def test_core_find_cid_by_label_fragment(core_client):
    pairs = _skip_on_qlever_hiccup(ql.core_find_cid_by_label_fragment, "ibuprofen", 25)
    assert isinstance(pairs, list)
...
def test_bio_measuregroup_roundtrip_smoke(bio_client):
    ...
    if not mg_rows:
        pytest.skip(
            "No /measuregroup/ resources in BIO index (load PubChem measuregroup TTLs)."
        )
    ...
    assert all(
        r["cid"]["value"].startswith(PUBCHEM_COMPOUND_NS + "CID") for r in rows_sc
    )
