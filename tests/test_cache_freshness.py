...
def test_refresh_is_scoped_and_reaches_source_workers(tmp_path, monkeypatch):
    ...
    with caching.source_cache_policy(force_refresh=True, max_age_s=3600):
        ...
        with ThreadPoolExecutor(1) as pool:
            assert pool.submit(copy_context().run, caching.load_json, tmp_path, 'source').result() is None
...
def test_context_ttl_uses_original_timestamp(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(rag, 'get_settings', lambda: SimpleNamespace(evidence_cache_ttl_hours=24))
...
def test_expired_and_forced_contexts_rebuild(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(rag, 'get_settings', lambda: SimpleNamespace(evidence_cache_ttl_hours=24, cache_backend='file'))
    calls = []
    ...
    assert len(calls) == 1
    ctx['meta']['evidence_assembled_at'] = (datetime.now(timezone.utc) - timedelta(days=2)).isoformat()
