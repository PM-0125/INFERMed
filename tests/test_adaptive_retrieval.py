...
class TestAdaptiveRetrieve:
    ...
    def test_expansion_on_low_relevance(self):
        ...
        results, metadata = adaptive_retrieve(
            "query", retrieve_fn, initial_k=5, min_relevance=0.5, max_k=20
        )
    ...
    def test_expansion_on_insufficient_results(self):
        ...
        results, metadata = adaptive_retrieve(
            "query", retrieve_fn, initial_k=5, min_results=5, max_k=20
        )
    ...
    def test_no_expansion_when_quality_good(self):
        ...
        results, metadata = adaptive_retrieve(
            "query", retrieve_fn, initial_k=5, min_relevance=0.5, min_results=3
        )

        # Should not expand
        assert (
            metadata["expanded"] is False
            or metadata["final_k"] == metadata["initial_k"]
        )
    ...
    def test_max_k_limit(self):
        ...
        results, metadata = adaptive_retrieve(
            "query", retrieve_fn, initial_k=5, min_relevance=0.5, max_k=10
        )
...
class TestAdaptiveRetrieveWithFallback:
    ...
    def test_primary_success(self):
        ...
        results, metadata = adaptive_retrieve_with_fallback(
            "query", primary_fn, None, initial_k=5
        )
    ...
    def test_fallback_usage(self):
        ...
        results, metadata = adaptive_retrieve_with_fallback(
            "query", primary_fn, fallback_fn, initial_k=5
        )
