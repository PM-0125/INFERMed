    ...
    try:
        ...
        result = {
            ...
            "qlever": {
                ...
                "enzymes_a": {
                    "substrate": len(
                        mech.get("enzymes", {}).get("a", {}).get("substrate", [])
                    ),
                    "inhibitor": len(
                        mech.get("enzymes", {}).get("a", {}).get("inhibitor", [])
                    ),
                    "inducer": len(
                        mech.get("enzymes", {}).get("a", {}).get("inducer", [])
                    ),
                },
                "enzymes_b": {
                    "substrate": len(
                        mech.get("enzymes", {}).get("b", {}).get("substrate", [])
                    ),
                    "inhibitor": len(
                        mech.get("enzymes", {}).get("b", {}).get("inhibitor", [])
                    ),
                    "inducer": len(
                        mech.get("enzymes", {}).get("b", {}).get("inducer", [])
                    ),
                },
            },
            "duckdb": {
            ...
            "api_enhancements": {
                ...
                "uniprot_enriched_targets": any(
                    "(" in str(t) and ")" in str(t)
                    for t in (mech.get("targets_a", []) + mech.get("targets_b", []))
                ),
            ...
            "pkpd": {
                ...
                "has_enhanced_pathways": "Enhanced pathways"
                in pkpd.get("pd_summary", ""),
            },
            "caveats": len(caveats),
        ...
        print(f"\n📊 Drug Information:")
        print(
            f"  {drugA}: CID {result['drugs']['a']['cid']}, {result['drugs']['a']['synonyms']} synonyms"
        )
        print(
            f"  {drugB}: CID {result['drugs']['b']['cid']}, {result['drugs']['b']['synonyms']} synonyms"
        )

        print(f"\n🧬 QLever Data (CORE/BIO/DISEASE):")
        print(
            f"  Targets: {drugA}={result['qlever']['targets_a']}, {drugB}={result['qlever']['targets_b']}"
        )
        print(
            f"  Diseases: {drugA}={result['qlever']['diseases_a']}, {drugB}={result['qlever']['diseases_b']}"
        )
        print(
            f"  Enzymes {drugA}: S={result['qlever']['enzymes_a']['substrate']}, I={result['qlever']['enzymes_a']['inhibitor']}, Ind={result['qlever']['enzymes_a']['inducer']}"
        )
        print(
            f"  Enzymes {drugB}: S={result['qlever']['enzymes_b']['substrate']}, I={result['qlever']['enzymes_b']['inhibitor']}, Ind={result['qlever']['enzymes_b']['inducer']}"
        )

        print(f"\n💾 DuckDB Data:")
        print(f"  PRR: {result['duckdb']['prr']}")
        print(
            f"  Side Effects: {drugA}={result['duckdb']['side_effects_a']}, {drugB}={result['duckdb']['side_effects_b']}"
        )
        print(
            f"  DILI: {drugA}={result['duckdb']['dili_a']}, {drugB}={result['duckdb']['dili_b']}"
        )
        print(
            f"  DICT: {drugA}={result['duckdb']['dict_a']}, {drugB}={result['duckdb']['dict_b']}"
        )

        print(f"\n🏥 OpenFDA/FAERS Data:")
        print(
            f"  Reactions: {drugA}={result['openfda']['faers_a']}, {drugB}={result['openfda']['faers_b']}, Combo={result['openfda']['faers_combo']}"
        )

        print(f"\n✨ API Enhancements (UniProt, KEGG, Reactome):")
        api = result["api_enhancements"]
        print(
            f"  KEGG Pathways: {drugA}={api['kegg_pathways_a']}, {drugB}={api['kegg_pathways_b']}, Common={api['common_pathways']}"
        )
        print(
            f"  UniProt Enrichment: {'✅' if api['uniprot_enriched_targets'] else '❌'}"
        )
        print(
            f"  PubChem PK Data: {drugA}={api['pk_data_a']}, {drugB}={api['pk_data_b']}"
        )
...
def main():
    ...
    total_targets = sum(
        r.get("qlever", {}).get("targets_a", 0)
        + r.get("qlever", {}).get("targets_b", 0)
        for _, _, r in results
        if r.get("success")
    )
    total_diseases = sum(
        r.get("qlever", {}).get("diseases_a", 0)
        + r.get("qlever", {}).get("diseases_b", 0)
        for _, _, r in results
        if r.get("success")
    )
    ...
    pairs_with_prr = sum(
        1
        for _, _, r in results
        if r.get("success") and r.get("duckdb", {}).get("prr") is not None
    )
    print(f"  Pairs with PRR data: {pairs_with_prr}/{successful}")

    print(f"\n🏥 OpenFDA Data Coverage:")
    pairs_with_faers = sum(
        1
        for _, _, r in results
        if r.get("success")
        and (
            r.get("openfda", {}).get("faers_a", 0) > 0
            or r.get("openfda", {}).get("faers_b", 0) > 0
        )
    )
    ...
    pairs_with_kegg = sum(
        ...
        if r.get("success")
        and (
            r.get("api_enhancements", {}).get("kegg_pathways_a", 0) > 0
            or r.get("api_enhancements", {}).get("kegg_pathways_b", 0) > 0
        )
    )
    pairs_with_uniprot = sum(
        1
        for _, _, r in results
        if r.get("success")
        and r.get("api_enhancements", {}).get("uniprot_enriched_targets", False)
    )
    pairs_with_enhanced_pd = sum(
        1
        for _, _, r in results
    ...
    for drugA, drugB, result in results:
        ...
        if result.get("success"):
            ...
            print(
                f"      QLever: T={ql.get('targets_a', 0)}+{ql.get('targets_b', 0)}, "
                f"D={ql.get('diseases_a', 0)}+{ql.get('diseases_b', 0)}"
            )
            db = result.get("duckdb", {})
            print(
                f"      DuckDB: PRR={db.get('prr', 'N/A')}, SE={db.get('side_effects_a', 0)}+{db.get('side_effects_b', 0)}"
            )
            of = result.get("openfda", {})
            print(
                f"      OpenFDA: {of.get('faers_a', 0)}+{of.get('faers_b', 0)}+{of.get('faers_combo', 0)}"
            )
            api = result.get("api_enhancements", {})
            print(
                f"      API Enhancements: KEGG={api.get('kegg_pathways_a', 0)}+{api.get('kegg_pathways_b', 0)}, "
