...
class TestReranker:
    ...
    def test_rerank_with_scores(self, reranker):
        ...
        results = reranker.rerank_with_scores(
            query, scored_docs, top_k=2, combine_with_original=True
        )
    ...
    def test_rerank_with_scores_no_combination(self, reranker):
        ...
        query = "warfarin"
        scored_docs = [("Warfarin doc", 0.9), ("Other doc", 0.8)]

        results = reranker.rerank_with_scores(
            query, scored_docs, combine_with_original=False
        )

        assert len(results) == 2
        # Scores should be from reranking only
        assert all(score != 0.9 for _, score in results) or all(
            score != 0.8 for _, score in results
        )
